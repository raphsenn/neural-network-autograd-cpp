...
TEST(LearnsToHalfeRealNumbers, NeuralNetwork) {
  ...
  Matrix<float> X_train =
      std::vector<std::vector<float>>({{0}, {1}, {2}, {3}, {4}, {5}, {6}, {7}});
  Matrix<float> y_train = std::vector<std::vector<float>>(
...
// ____________________________________________________________________________
TEST(NotGate, NeuralNetwork) {
  // This neural network learns how to solve the NOT-Gate problem.
  Matrix<float> X_train = std::vector<std::vector<float>>({{0}, {1}});
  Matrix<float> y_train = std::vector<std::vector<float>>({{1}, {0}});
  NeuralNetwork<float> nn(std::vector<size_t>({1, 1}),
                          std::vector<Activation>({Activation::sigmoid}), 0.1f,
                          InitState::RANDOM);
  nn.train(X_train, y_train, 1, 0.1f, 10000, false);
