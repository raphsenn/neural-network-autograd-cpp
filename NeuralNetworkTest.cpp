...
TEST(LearnsToHalfeRealNumbers, NeuralNetwork) {
  ...
  NeuralNetwork<float, float> nn(std::vector<size_t>({1, 1}), std::vector<Activation>({Activation::linear}), 0.01f, InitState::RANDOM);
...
TEST(AndGate, NeuralNetwork) {
  ...
  NeuralNetwork<float, float> nn(std::vector<size_t>({2, 1}), std::vector<Activation>({Activation::sigmoid}), 0.1f, InitState::RANDOM);
  nn.train(X_train, y_train,1, 0.1f, 10000, false);
