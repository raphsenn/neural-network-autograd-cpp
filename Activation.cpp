...
Matrix<T> sigmoid(const Matrix<T>& X) {
  ...
  for (size_t row = 0; row < X.getRows(); ++row) {
    for (size_t col = 0; col < X.getCols(); ++col) {
      result[row][col] = value<T>::one() / (value<T>::one() + value<T>::e(-X.getValue(row, col)));
    }
  }
  return result;
 }
...
Matrix<T> sigmoid_derivative(const Matrix<T>& X) {
  ...
  for (size_t row = 0; row < X.getRows(); ++row) {
     for (size_t col = 0; col < X.getCols(); ++col) {
      result[row][col] = (value<T>::one() / (value<T>::one() + value<T>::e(-X.getValue(row, col)))) * (value<T>::one() - (value<T>::one() / (value<T>::one() + value<T>::e(-X.getValue(row, col)))));
    }
  }
  return result;
}
